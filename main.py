...
def draw():
    ...
    if game.state == State.MAIN_MENU:
        ...
        # draw version text
        screen.blit(game.main_menu.version_text, (screen.get_width() - game.main_menu.version_text.get_width(),
                                                  screen.get_height() - game.main_menu.version_text.get_height()))
    # if game state is second menu
    elif game.state == State.SECOND_MENU:
        # draw background image
        screen.blit(game.second_menu.background, (0, 0))
        # draw version text
        screen.blit(game.second_menu.version_text, (screen.get_width() - game.main_menu.version_text.get_width(),
                                                    screen.get_height() - game.main_menu.version_text.get_height()))
        # draw select text
...
def mouse_button_up():
    if game.state == State.MAIN_MENU:
        ...
        if game.second_menu.is_history_mode_button_pressed(pygame.mouse.get_pos()):
            if game.game_data.data_player['history_mode']['adv'] == 1:
                fade_out(screen.get_width(), screen.get_height(), 4)
...
def key_up(e):
    if game.state == State.HISTORY_MODE_START_HELP:
        if e.key == pygame.K_RETURN:
            game.hm_help1_menu.current_paragraph = game.hm_help1_menu.current_paragraph + 1
