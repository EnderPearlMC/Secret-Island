...
class City:
    ...
    def unlocked_items(self):
        """
            Must return the unlocked items of all the categories (Dictionnary)
    ...
    def __init__(self):
        ...
        # selected_tile_image | Stores the selected tile when building (Can place the tile : blue / Can't : red)
        self.selected_tile_image = pygame.image.load("assets/images/historyMode/cities/selectedRight.png")
        # it's rect
        self.selected_tile_rect = self.selected_tile_image.get_rect()
        self.selected_tile_rect.x = 0
        self.selected_tile_rect.y = 0
        ...
        self.build_categories = [
            {
                ...
                "items": [
                    ...
                    {
                        "name": "3 - Stone Brick",
                        "image_path": "assets/images/historyMode/cities/tiles/stonebrick.png",
                        "price": 10
                    },
                    {
                        "name": "4 - Water",
                    ...
                    {
                        "name": "5 - Sand",
                        "image_path": "assets/images/historyMode/cities/tiles/sand.png",
                        "price": 15
                    }
                ],
                "shown": False
            },
            {
                ...
                "items": [
                    {
                        "name": "1 - Town Hall",
                        "image_path": "assets/images/historyMode/cities/builds/townHallLogo.png",
                        "price": 150
                    }
                ],
                "shown": False
        ...
        # builds | Stores all the builds of the city
        self.builds = []

        # builds_infos | Stores informations of every build
        self.builds_infos = {
            "town_hall": {
                "size": [8, 8]
            }
        ...
        self.build_panel_category_return = pygame.image.load('assets/images/back.png')
        ...
        # build_panel_on | True : the building panel is up / False : down
        self.build_panel_on = True

        # BUILD CATEGORIES
        for c in self.build_categories:

            c['hovered_finished'] = False
            c['stop_hover_finished'] = False
            ...
            """
                ITEMS
            """
            for idx, i in enumerate(c['items']):

                i['hovered_finished'] = False
                i['stop_hover_finished'] = False
                i['selected'] = False
                i['stop_select_finished'] = False

                # unlock
                if idx in self.unlocked_items()[c['name']]:
                    i['unlocked'] = True
                else:
                    i['unlocked'] = False
                    i['locked_image'] = pygame.image.load('assets/images/lock.png')
                ...
                # add price text
                i['price_text'] = self.font4.render(str(i['price']), True, (255, 255, 255))

                # add coin image
                i['coin_image'] = pygame.image.load('assets/images/mainMenu/coin.png')
    ...
    def draw_city(self, screen, game):
        ...
        if not self.loaded:
            ...
            # add builds into their list
            self.builds = game.game_data.data_player['history_mode']['region'][self.yaml_name()]['city']['builds']

            for b in self.builds:
                b['image'] = pygame.image.load(b['image_path'])
                b['image'] = pygame.transform.scale(b['image'], (self.builds_infos[b['type']]['size'][0] * self.TILES_SIZE, self.builds_infos[b['type']]['size'][1] * self.TILES_SIZE))
            ...
            self.build_panel_category_return = pygame.transform.scale(self.build_panel_category_return,
                                                        (int(screen.get_width() / 15), int(screen.get_height() / 13)))
            ...
            for c in self.build_categories:
                ...
                c['text'] = pygame.transform.scale(c['text'], (
                    int(c['button_image'].get_width() / 1.5), int(c['button_image'].get_height() / 5)))
                ...
                for i in c['items']:
                    ...
                    i['price_text'] = pygame.transform.scale(i['price_text'], (
                    len(str(i['price'])) * 13, int(i['button_image'].get_height() / 5)))

                    i['coin_image'] = pygame.transform.scale(i['coin_image'], (
                    int(i['button_image'].get_width() / 5.3), int(i['button_image'].get_height() / 5)))

                    if not i['unlocked']:
                        i['locked_image'] = pygame.transform.scale(i['locked_image'], (int(i['button_image'].get_width() / 2), int(i['button_image'].get_height() / 2)))
        ...
        self.selected_tile_image = pygame.image.load("assets/images/historyMode/cities/selectedWrong.png")

        # update the if an item of a category is selected
        for c in self.build_categories:
            for idx, val in enumerate(c['items']):
                if c['name'] == "Ground":
                    if self.selected_ground_tile is not None:
                        if idx != self.selected_ground_tile - 1:
                            val['selected'] = False

        # reload button of categories
        for c in self.build_categories:
            c['button_image'] = pygame.transform.scale(c['button_image'],
                                                       (int(screen.get_width() / 8), int(screen.get_height() / 6)))
            for i in c['items']:
            ...
            if 0 <= x_tile < len(map[y_tile]):
                if self.selected_ground_tile is not None:
                    if map[y_tile][x_tile] != self.selected_ground_tile:
                        if game.game_data.data_player['money'] + \
                                self.build_categories[0]['items'][map[y_tile][x_tile] - 1]['price'] >= \
                                self.build_categories[0]['items'][self.selected_ground_tile - 1]['price']:
                            self.selected_tile_image = pygame.image.load(
                                "assets/images/historyMode/cities/selectedRight.png")
        self.selected_tile_image = pygame.transform.scale(self.selected_tile_image, (self.TILES_SIZE, self.TILES_SIZE))

        # get rects

        self.move_selected_tile_image()

        for b in self.builds:
                b['rect'] = b['image'].get_rect()
                b['rect'].x = b['x'] * self.TILES_SIZE + self.camera_pos[0]
                b['rect'].y = b['y'] * self.TILES_SIZE + self.camera_pos[1]
        ...
        self.build_panel_rect = self.build_panel.get_rect()
        if self.build_panel_on:
            self.build_panel_rect.x = 10
            self.build_panel_rect.y = screen.get_height() - self.build_panel.get_height()
        else:
            self.build_panel_rect.x = 10
            self.build_panel_rect.y = screen.get_height() - self.build_panel.get_height() / 4

        self.build_panel_minimize_rect = self.build_panel_minimize.get_rect()
        self.build_panel_minimize_rect.x = screen.get_width() - self.build_panel_minimize.get_width() - 25
        self.build_panel_minimize_rect.y = self.build_panel_rect.y + self.build_panel.get_height() / 14

        self.build_panel_category_return_rect = self.build_panel_category_return.get_rect()
        self.build_panel_category_return_rect.x = self.build_panel_rect.x + 15
        self.build_panel_category_return_rect.y = self.build_panel_rect.y + self.build_panel.get_height() / 2.4
        ...
        for idx, val in enumerate(self.build_categories):
            ...
            for idx2, val2 in enumerate(val['items']):
                ...
                val2['button_rect'].x = (self.build_panel_rect.x + self.build_panel.get_width() / 10) + idx2 * val2['button_image'].get_width()
        ...
        for c in self.build_categories:
            if not self.is_one_category_shown:
                ...
                screen.blit(c['text'], (c['button_rect'].x + c['button_image'].get_width() / 7,
                                        c['button_rect'].y + c['button_image'].get_height() / 12))
            else:
                for i in c['items']:
                    if c['shown']:
                        screen.blit(self.build_panel_category_return, self.build_panel_category_return_rect)
                        screen.blit(i['button_image'], i['button_rect'])
                        screen.blit(i['image'],
                                    (i['button_rect'].x + i['button_image'].get_width() / 3,
                                     i['button_rect'].y + i['button_image'].get_height() / 3.5))
                        screen.blit(i['text'], (i['button_rect'].x + i['button_image'].get_width() / 7,
                                                i['button_rect'].y + i['button_image'].get_height() / 12))
                        screen.blit(i['price_text'], (
                        i['button_rect'].x + i['button_image'].get_width() / 2 - i['price_text'].get_width() / 2,
                        i['button_rect'].y + i['button_image'].get_height() - i['price_text'].get_height() - 30))
                        screen.blit(i['coin_image'], (
                            i['button_rect'].x + i['button_image'].get_width() / 2 - i['coin_image'].get_width() / 2,
                            i['button_rect'].y + i['button_image'].get_height() - i['price_text'].get_height() - 5))
                        if not i['unlocked']:
                            screen.blit(i['locked_image'], (i['button_rect'].x + i['button_image'].get_width() / 2 - i['locked_image'].get_width() / 2,
                                                            i['button_rect'].y + i['button_image'].get_height() / 2 - i['locked_image'].get_height() / 2))
        ...
        if 0 <= y_tile < len(map):
            if 0 <= x_tile < len(map[y_tile]):
                if self.can_camera_move:
                    if self.selected_ground_tile is not None:
                        screen.blit(self.selected_tile_image, self.selected_tile_rect)
    ...
    def update_screen_resize(self, screen):
        ...
        self.build_panel_minimize = pygame.transform.scale(self.build_panel_minimize, (
            int(screen.get_width() / 34), int(screen.get_height() / 29)))
        self.build_panel_category_return = pygame.transform.scale(self.build_panel_category_return,
                                                                  (int(screen.get_width() / 15),
                                                                   int(screen.get_height() / 13)))
        ...
        for c in self.build_categories:
            ...
            for i in c['items']:
                ...
                if i['selected']:
                    i['button_image'] = pygame.image.load('assets/images/historyMode/cities/buildCategoryButtonSelected.png')
                else:
                    i['button_image'] = pygame.image.load('assets/images/historyMode/cities/buildCategoryButton.png')
    ...
    # when hover event
    def hover(self, pos):
        if self.build_panel_rect.collidepoint(pos) or self.return_button_rect.collidepoint(pos):
        ...
        for c in self.build_categories:
            if c['button_rect'].collidepoint(pos):
                c['stop_hover_finished'] = False
                if not c['hovered_finished']:
                    c['button_image'] = pygame.image.load('assets/images/historyMode/cities/buildCategoryButtonHovered.png')
                    c['hovered_finished'] = True
            else:
                c['hovered_finished'] = False
                if not c['stop_hover_finished']:
                    c['button_image'] = pygame.image.load('assets/images/historyMode/cities/buildCategoryButton.png')
                    c['stop_hover_finished'] = True
            for i in c['items']:
                if i['button_rect'].collidepoint(pos):
                    i['stop_hover_finished'] = False
                    if not i['hovered_finished'] and i['unlocked'] and not i['selected']:
                        i['button_image'] = pygame.image.load(
                            'assets/images/historyMode/cities/buildCategoryButtonHovered.png')
                        i['hovered_finished'] = True
                elif not i['selected']:
                    i['hovered_finished'] = False
                    if not i['stop_hover_finished']:
                        i['button_image'] = pygame.image.load(
                            'assets/images/historyMode/cities/buildCategoryButton.png')
                        i['stop_hover_finished'] = True
    ...
    def mouse_button_down(self, e, pos):
        ...
        if e.button == 1:
            ...
            # minimize building panel
            if self.build_panel_minimize_rect.collidepoint(pos):
                if self.build_panel_on:
                    self.build_panel_on = False
                else:
                    self.build_panel_on = True
            ...
            for c in self.build_categories:
                if self.is_one_category_shown:
                    if c['shown']:
                        for idx, val in enumerate(c['items']):
                            if self.build_panel_category_return_rect.collidepoint(pos):
                                c['shown'] = False
                                self.is_one_category_shown = False
                                self.selected_ground_tile = None
                                c['selected'] = False
                                val['button_image'] = pygame.image.load('assets/images/historyMode/cities/buildCategoryButton.png')
                            if val['button_rect'].collidepoint(pos):
                                if c['name'] == "Ground":
                                    if val['unlocked']:
                                        self.selected_ground_tile = idx + 1
                                        val['selected'] = True
                                        val['button_image'] = pygame.image.load('assets/images/historyMode/cities/buildCategoryButtonSelected.png')
    ...
    def add_tiles_images(self):
        self.tiles_images = [
            ...
            pygame.transform.scale(pygame.image.load("assets/images/historyMode/cities/tiles/dirt.png").convert(),
                                   (self.TILES_SIZE, self.TILES_SIZE)),
            pygame.transform.scale(pygame.image.load("assets/images/historyMode/cities/tiles/stonebrick.png").convert(),
                                   (self.TILES_SIZE, self.TILES_SIZE)),
            pygame.transform.scale(pygame.image.load("assets/images/historyMode/cities/tiles/water.png").convert(),
                                   (self.TILES_SIZE, self.TILES_SIZE)),
            pygame.transform.scale(pygame.image.load("assets/images/historyMode/cities/tiles/sand.png").convert(),
                                   (self.TILES_SIZE, self.TILES_SIZE))
        ]
                    ...
                    if map[y_tile][x_tile] != self.selected_ground_tile:
                        if game.game_data.data_player['money'] + \
                            ...
                            game.game_data.data_player['money'] += \
                                self.build_categories[0]['items'][map[y_tile][x_tile] - 1]['price']
                            game.game_data.data_player['history_mode']['region'][self.yaml_name()]['city']['map'][
                                y_tile][x_tile] = self.selected_ground_tile
                            game.game_data.make_datas_as_file()
                        else:
                            pass

    def move_selected_tile_image(self):
        x_tile = math.floor((pygame.mouse.get_pos()[0] - self.camera_pos[0]) / self.TILES_SIZE)
        y_tile = math.floor((pygame.mouse.get_pos()[1] - self.camera_pos[1]) / self.TILES_SIZE)
        self.selected_tile_rect.x = x_tile * self.TILES_SIZE + self.camera_pos[0]
        self.selected_tile_rect.y = y_tile * self.TILES_SIZE + self.camera_pos[1]
